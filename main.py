...
class UIOperate(QMainWindow, Ui_MainWindow):
    ...
    def __init__(self, parent=None):
        ...
        # 展示推理图像按钮
        # self.show_img.clicked.connect(self.load_infer_Image)
    ...
    def showimg(self, img):  #img是appedn后的list类型
        self.img3D = img
    ...
    def view3D(self):  # 预测结果3D模型展示   d，h，w
        ...
        matrix = self.concat_along_new_dim(self.img3D)
        # print(matrix.shape)   #5张图片，(5, 512, 512)
    ...
    def exitNet(self):  # 不知道可不可以直接退出——这个直接退出整个系统
        ...
        ctypes.windll.kernel32.TerminateThread(self.inferNet.handle, 0)
        print('终止线程')
        # self.inferNet.quit()
        self.time_q.stop()   #不能重新启动
        self.progressBar_2.reset()

'''这个加上了handle，删除了stop函数'''
class loadNetThread(QThread):
    ...
    def __init__(self):
        ...
        self.imgArr = []
        self.zeronumpy = np.zeros((512, 512))

    def run(self):  #重写QThread类的方法
        # self.flag = 1
        try:
        ...
        if self.flag == 1:
            ...
            for i in range(len(self.infer.img_path_glob)):
                ...
                # print(new_img.shape)
                if(np.array_equal(self.zeronumpy ,img)==False):
        ...
        self.imgSignal.emit(self.imgArr)  #发送最后的list类型的数据
'''后面没有改了'''
